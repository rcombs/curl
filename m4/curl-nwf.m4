...
if test "x$OPT_NETWORKFRAMEWORK" != xno; then
  if test "x$OPT_NETWORKFRAMEWORK" != "xno" &&
    ...
    AC_MSG_RESULT(yes)
    AC_DEFINE(USE_NWF, 1, [enable Network.framework])
    ...
    NETWORKFRAMEWORK_LDFLAGS='-framework CoreFoundation -framework CoreServices -framework Security -framework Network'
